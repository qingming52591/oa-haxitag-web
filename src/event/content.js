...
export const getContent = async (pagination, topic = '', type = '') => {
    ...
    let r = await axios.get(`${config.API_OA_BASE}/v1/contents/list?page_num=${pagination.current}&page_size=${pagination.pageSize}&topic=${topic}&type=${type}`, {headers: {Authorization: state.user.info.token}})
